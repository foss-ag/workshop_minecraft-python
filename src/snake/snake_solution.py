...
def gameLoop():
    ...
    while not done:
        ...
        while gameOver == True:
            show_end_screen(snake_length)

        ##### Schriit 2
        ####################### Deinen Code hier einfügen ########################
        e_type, e_key = get_pressed_button()
        if e_type is not None:
            ...
            if e_type == pygame.KEYDOWN:
                ...
                if e_key == pygame.K_LEFT and snake_x_change <= 0:
                    # negative x motion indicates left movement
                    snake_x_change = -block_size
                    # Reset y motion to prevent diagonal motion
                    snake_y_change = 0
                elif e_key == pygame.K_RIGHT and snake_x_change >= 0:
                    snake_x_change = block_size
                    snake_y_change = 0
                elif e_key == pygame.K_UP and snake_y_change <= 0:
                    snake_x_change = 0
                    snake_y_change = -block_size
                elif e_key == pygame.K_DOWN and snake_y_change >= 0:
                    snake_x_change = 0
                    snake_y_change = block_size
        ##########################################################################
        ...
        snake_segment = [snake_x, snake_y]
        snake_list.append(snake_segment)

        if len(snake_list) > snake_length:
            del snake_list[0]

        gameOver = snake_bit(snake_list, snake_segment) or snake_left_screen(snake_x, snake_y)
